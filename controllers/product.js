...
export const createProduct = async (req, res) => {
  const { name, barcode, description, price, cost, image, quantity } = req.body;
  try {
    const product = await Product.create({ name, barcode, description, price, cost, image, quantity });
...
export const updateProductById = async (req, res) => {
  ...
  try {
    const product = await Product.findByPk(id);
    if (product) {
      await product.update({ name, barcode, description, price, cost, image, quantity });
      res.status(200).json(product);
    } else {
      res.status(404).json({ error: "Produto não encontrado" });
    }
